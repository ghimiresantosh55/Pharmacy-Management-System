...
class PurchaseOrderMasterHistorySerializer(serializers.ModelSerializer):
    ...
    supplier_first_name = SerializerMethodField()
    supplier_middle_name = SerializerMethodField()
    supplier_last_name = SerializerMethodField()
    ...
    def get_supplier_first_name(self,purchase_order):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
    ...
    def get_supplier_middle_name(self,purchase_order):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
            try:
                middle_name = Supplier.objects.get(id=purchase_order.supplier_id).middle_name
                return middle_name
            except:
                return None
    
    def get_supplier_last_name(self,purchase_order):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
            try:
                last_name = Supplier.objects.get(id=purchase_order.supplier_id).last_name
                return last_name
            except:
                return None
...
class PurchaseMasterHistorySerializer(serializers.ModelSerializer):
    ...
    def get_supplier_first_name(self,purchase_master):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
    ...
    def get_supplier_middle_name(self,purchase_master):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
            try:
                middle_name = Supplier.objects.get(id=purchase_master.supplier_id).middle_name
                return middle_name
            except:
                return None
    
    def get_supplier_last_name(self,purchase_master):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
            try:
                last_name = Supplier.objects.get(id=purchase_master.supplier_id).last_name
                return last_name
            except:
                return None
