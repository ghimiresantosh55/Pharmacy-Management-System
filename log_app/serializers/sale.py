...
class SaleMasterHistorySerializer(serializers.ModelSerializer):
    ...
    customer_middle_name = SerializerMethodField()
    ...
    def get_customer_first_name(self,sale_master):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
    ...
    def get_customer_middle_name(self,sale_master):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
            try:
                middle_name = Customer.objects.get(id=sale_master.customer_id).middle_name
                return middle_name
            except:
                return None
    
    def get_customer_last_name(self,sale_master):
        request = self.context.get('request')
        schema = tenant_schema_from_request(request)
        with connections['default'].cursor() as c:
            c.execute(f'SET search_path to {schema}')
            try:
                last_name = Customer.objects.get(id=sale_master.customer_id).last_name
                return last_name
            except:
                return None
