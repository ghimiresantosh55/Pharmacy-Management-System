...
class PartyPayment(NowTimestampAndUserModel):
    PAYMENT_TYPE = (
        (1, "PAYMENT"),
        (2, "REFUND"),
    )

    purchase_master = models.ForeignKey(PurchaseMaster, on_delete=models.PROTECT, related_name= "partyclearance")
...
class BasicPartyPayment(NowTimestampAndUserModel):
    PAYMENT_TYPE = (
        (1, "PAYMENT"),
        (2, "RETURN"),
    )  
    ...
    amount = models.DecimalField(max_digits=12, decimal_places=2, help_text="Maximum value 9999999999.99, min_value=0.0")
    receipt_no = models.CharField(max_length=20,blank=True, null= True ,help_text="Receipt no can have max of 20 characters and blank = true")
...
class BasicPartyPaymentDetail(NowTimestampAndUserModel):
